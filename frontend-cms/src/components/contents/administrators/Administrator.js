...
export default function Administrators(){
    ...
    const dataAdministrators = async()=>{
        //Creating dataSet (dataTable)
        const getAdministrators = await getData();
        const dataSet = [];
        ...
        //Executing datatable    
        $(function () { //document ready removed
            $('.table').DataTable({
                retrieve: true, //this due to error. See: https://datatables.net/manual/tech-notes/3
                data: dataSet,
                columns: [
                    { title: "#" },
                    { title: "Usuario" },
                    ...
                    render: function(data){ //Returning the buttons to the "Acciones" column
                        ...
                        <a href="" class="deleteInput" data="${data}">
...
//Remember that we make petitions outside component export
//Petition Get for Administrators
const getData = ()=>{
    const url = `${routeAPI}/mostrar-administradores`;
    //This is a private petition, because you need to be logged in to do this petition, so you have to send the token
